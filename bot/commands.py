...
class BotCommands:
    ...
    command_help = {
        ...
        'give': 'Confere uma insígnia à um Treinador | alias: [add_badge] | Parâmetros: [@usuario_desafiante] |\n Ex:\n\t `>>give @beelzebruno`\nPRIVILÉGIOS: Apenas membros  `gym_leader` podem usar este recurso!',
        ...
        'report': 'Registra o resultado de uma partida entre um líder de ginásio e um treinador | alias: [rp, res] | Parâmetros: [@usuario, resultado do líder (v/f)] | \nEx: Líder reporta vitória contra  beelzebruno \n\t `>>res @beelebruno` v \nEx: Líder reporta derrota contra  beelzebruno\n\t `>>res @beelebruno` f \nPRIVILÉGIOS: Apenas membros `gym_leader` podem usar este recurso!',
    ...
    @staticmethod
    def add_badge(leader_id, member_id):
        
        db = ABP_DB(db_connection())
        data = BotCommands.get_leader(leader_id)
        leader = GymLeader(*data)
